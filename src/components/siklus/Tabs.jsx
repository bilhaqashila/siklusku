...
function Tabs({ value, defaultValue = "dashboard", onValueChange, children, className }) {
  ...
  return (
    <TabsCtx.Provider value={ctx}>
      <div data-slot="tabs" className={["flex w-full flex-col gap-4", className || ""].join(" ")}>
        {children}
      </div>
    </TabsCtx.Provider>
  );
...
export function TabsList({ children, className, ...props }) {
  return (
    <div
      ...
      className={[
        "mx-auto mb-6 flex max-w-md items-center justify-center rounded-full bg-pink-50 p-1",
        "dark:bg-slate-800",
        className || "",
      ].join(" ")}
      {...props}
